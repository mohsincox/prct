			...
			$.ajax({
				...
				success: function (response) {
					if(response.download_csv_data.length > 0){
						var table_data = [];
						table_data.push(["Invoice No","Item Name","Serial No","Date"]);

						for(var i=0; i<response.download_csv_data.length; i++){
							table_data.push([JSON.stringify(response.download_csv_data[i].invoice_no), JSON.stringify(response.item.name), JSON.stringify(response.download_csv_data[i].slno), JSON.stringify(response.download_csv_data[i].created_at)]);
						}
