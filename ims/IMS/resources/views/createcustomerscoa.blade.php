...
<div id="content">
			...
			<div class="grid_12 full_block">
				<div class="widget_wrap">
					<div class="widget_top">
						...
						<h6>Add Customer Account Information</h6>
					</div>
					<div class="widget_content">
						<h3>Create New Customer Account</h3>
						<form action="/IMS/coa/customers/register" method="post" class="form_container left_label">
						<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<input type="hidden" name="userid" value="{{ Auth::id() }}">
							<ul>				
								<li>
										<div class="form_grid_6">		
									<label for="name" class="field_title">Cuatomer Name</label>
									<div class="form_input">
							<select class="chzn-select" style=" width:100%" name="name" onchange="fetch_select(this.value)"  > 
							  <?php 

								echo "<option value='' Selected>Select</option>";
								foreach($customers as $gg){
									if($gg->coastatus==0){
								...
								<div class="form_grid_6">
									...
									<div class="form_input">
										<input name="code" type="text" tabindex="1" id="code" value="<?php echo"C-".(rand()); ?>" readonly="" />
									</div>
								</div>
	...
	<script type="text/javascript">
		function fetch_select(customer_id)
		{
			var arr = customer_id.split("+");
			//alert(arr[0])
			$_token = "{{ csrf_token() }}";
			$.ajax({
				type: 'post',
